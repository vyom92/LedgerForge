...
nonisolated enum BackupCompatibility {
    static let supportedSchemaVersion = 20
    ...
    static func migrations(for version: Int) throws -> [Migration] {
        guard [17, 18, 19, 20].contains(version), allMigrations.last?.version == 20,
              allMigrations.map(\.version) == Array(1...20) else { throw BackupError.incompatible }
    ...
    static func verifyDatabase(_ db: SQLiteDatabase, schemaVersion: Int = supportedSchemaVersion) throws {
        ...
        let inventory = try (schemaVersion == 17 ? v17Inventory : schemaVersion == 18 ? v18Inventory : schemaVersion == 19 ? v19Inventory : v20Inventory).get()
    ...
    /// Receipt-owned startup and isolated V17/V18/V19 candidates share this exact
    ...
    static func prepareCandidate(package: URL, manifest: BackupManifest, destination: URL) throws -> String {
        ...
        do {
            ...
            if manifest.schemaVersion < 20 { try upgradeSupportedCandidateIfNeeded(db) }
