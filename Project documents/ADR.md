...
- **Decision standing:** Accepted and implemented in Sprint 31.
...
- **Implementation:** Migration V6, StatementDate, date-role/timezone evidence, parser-profile provenance, source ordinal/digest relationships and strict hydration are operational.
- **Current qualification:** Sprint 53 extended the same approved shared-layout Axis bank-account CSV profile to supplied NRO evidence without changing this ADR or Migration V6.
...
Sprint 78B adds the exact Axis App PDF, traditional PDF and XLSX credit-card
families. The authentic Jan–Jul corpus proves a different valid use of the same
domain: one liability account may own accepted card statements while the source
proves zero durable card instruments and zero statement instrument sections.
Axis therefore requires an explicit liability-account decision and account-level
financial rows without inventing ownership from a masked suffix, holder text,
filename or import order. It also establishes representation-neutral exact
financial multiset equivalence across App PDF, traditional PDF and XLSX, with
additional exact source-order/narration equality only for App PDF ↔ XLSX.
...
For the exact Axis Sprint 78B profiles, current authentic sources prove no
account strong identifier and no instrument sections. Import review therefore
always requires an explicit liability-account decision: reuse one eligible Axis
INR credit-card liability account or create a separate one. Even exactly one
eligible account is never auto-selected. Axis evidence cannot create, reuse or
relate a `CardInstrument`; instrument-oriented choices fail closed at the
persistence boundary. A physical replacement card may continue the same
liability account only because the user explicitly chooses that existing
liability account. A changed card suffix does not itself create a new account,
a new instrument, a replacement relationship or lifecycle transition.
...
For exact `axis.credit-card.pdf@1` and `axis.credit-card.xlsx@1`, native INR rows
are account-level and use `CardLiabilityEffect`. Cross-format financial identity
is exact multiset equality, including multiplicity, over financial date +
liability effect + native currency + exact Money. App PDF and XLSX additionally
preserve exact financial source order and narration after only accepted inert
Unicode/whitespace normalization. Traditional PDF order need not equal App/XLSX
order. App PDF/XLSX chronology is the source-owned selected statement month;
statement day/period, filename-derived cycles and count-derived cycles are not
invented. Active Loans Summary is excluded from accepted transaction history
and does not establish loan/installment history.

### Atomic persistence and Migrations V12–V15
...
Additive Migration V15 leaves V1–V14 SQL immutable and extends the shared card
evidence graph only for the accepted Axis requirements. It permits the exact
Axis statement/summary forms and representation-neutral semantic events needed
for multiplicity-safe cross-format support while preserving zero instrument and
zero section ownership for Axis. Source-owned selected statement month may be
retained where proved. V15 performs no speculative Axis identity, instrument,
loan, summary-family or historical backfill and preserves SQLite/In-Memory
validation parity and zero-residue rejection.
...
For the exact Axis Sprint 78B profiles, semantic equivalence is representation-
neutral and multiplicity-safe over financial date, `CardLiabilityEffect`, native
currency and exact Money. The first accepted representation owns canonical
transactions; exact-equivalent later App PDF/XLSX/traditional representations
are durable supporting sources with zero duplicate canonical transactions. App
PDF and XLSX additionally require exact source financial order and normalized
narration equality; traditional PDF cross-format order is not an identity
requirement. Any financial multiset or required App-PDF/XLSX order/narration
mismatch fails closed with zero additional accepted financial residue.
...
account selects its current statement from source-owned chronology, never import
time. Amex/CBQ use their accepted statement date/period authority; Axis uses its
source-owned selected statement month where no statement day/period is proved.
Source amount owed is presented in runtime net-worth sign: an amount owed is
negative and a credit balance is positive. Older statements imported later
cannot replace newer source chronology.
...
Sprint 78B additionally accepts `axis.credit-card.pdf@1` and
`axis.credit-card.xlsx@1` only for the exact Jan–Jul Axis family established by
the approved private source corpus. The PDF profile covers exact App and
traditional PDF layouts through their separate structural recognizers and
separate canonical credential scopes; the XLSX profile uses the bounded OOXML
reader with locally vendored ZIPFoundation. App PDF tagged transaction-table
evidence is authoritative for that layout; traditional PDF may use the generic
positioned-evidence carrier. The accepted family preserves native INR, liability
effect, exact Money, source multiplicity, App-PDF/XLSX order and narration,
source-owned selected month, durable source provenance and zero fabricated Axis
instruments/sections. Generic XLSX, other Axis card layouts and structural
similarity remain unsupported.
...
ADR-044 does not authorize HDFC or other unaccepted card parsing, other Axis/
Amex/CBQ layouts, a generic XLSX reader, a generic card parser/profile framework,
generic masked account identity, rewards valuation or persistence, payment
allocation, bank-card payment matching, refund/reversal matching, Active Loans
history, generic installments/loans, calculated FX, invented fees/markup/tax,
automatic account merge/split, historical repair/backfill, cloud or multi-user
architecture, OCR, arbitrary encrypted PDFs or generic card support. ADR-043 CBQ
...
For included pre-salary QAR cash B, salary net N, included Qatar bills Cq, reserve K and configured fee F: P=B+N−Cq−K, signed capacity A=P−F, transferable principal T=max(0,A). For gross India bills Ci and explicitly included INR funds E: H=max(0,Ci−E), required principal R=ceil_to_QAR_cent(H/I) when H>0, otherwise R=0. The cost of meeting India is R+F only when R>0; margin G=P−cost. T×I uses full precision and rounds half-up only to final INR Money cents. Qatar-only outputs remain available without FX; H=0 needs no rate for R or G. Negative A/G remain visible. New-version calculations contain no planned-investment subtraction.

Named nonnegative deductions, reserve, row recurrence, temporary carry basis, optional first-due dates and fresh child IDs belong to the existing FundingPlan graph. Ordinary edited estimates and valid same-day dates seed only an absent next-month plan from the latest saved predecessor. One-off rows are omitted; a nonexistent day uses that month’s last day. The temporary bill exception retains the pre-reduction amount even if selected after editing. The later owner selection permits any month from September 2026 through December 2099 through separate Month/Year dropdowns. Session drafts own raw invalid text and selected-month edits; initialization alone cannot create an invisible seeded draft. No auto-save, automatic 25th switch or payslip-to-estimate overwrite is introduced. Legacy saved versions remain readable; adoption is an explicit unsaved draft change.

One injected app-owned AlDarReferenceSession serves Dashboard and Budget Planning, including WindowGroup consumers. It performs at most one in-flight request per INR/USD leg, opening freshness checks, a six-hour periodic trigger and one 60-second retry after failure. The public response remains a bounded exact positive decimal; USD's 27-place observation is not narrowed through Double. Derived six-direction labels use the two raw legs locally. Each leg owns its real fetch-success time; crosses use the older dependency and >24-hour evidence is stale. Failure cannot advance the success time. A versioned two-record local public cache is separate from appearance and durable plans. Dashboard shows provider references without a manual Refresh control; Budget Planning retains one. There is no new provider observation time, executable quote, fee inference or reverse retail quote claim.

New months always start in Al Dar mode. Shared INR automatically updates only the eligible active draft, with generation, invalid-text, historical-plan and manual-mode guards. A saved same-month manual override persists; Use Al Dar exits it. Atomic Save records the exact effective evidence/mode used. Public cache writes do not save a financial plan. Additive V19 contains worksheet/effective-reference fields; V20 adds the subsequently requested optional bill date because V19 had already opened in Xcode. V1–V18 remain immutable. [ADR-047](#adr-047) owns exact backup compatibility. [The existing work note](Work%20notes/Salary_and_current_AlDar.md#sprint-95-candidate) owns verification and limits.

## Historical accepted alignment — Sprint 94, 2026-09-15

The owner approved this architecture, authorized implementation and bounded verification, and now accepts Sprint 94 under `SPRINT_94_CURRENT_AL_DAR_AND_THIS_MONTH_PLANNING_ACCEPTED`. The [accepted outcome](Archive/Accepted%20outcomes/Sprints_90-99.md#sprint-94) records exact evidence and limits; closure changes no architecture or executable behavior. This alignment supersedes the external-reference planning gate in the historical notes below. The current product is a **QAR-1 current Al Dar unit planning reference**, indicative and used to estimate a transfer; historical amount-specific quotes are not the current workflow. The UI displays exactly two decimal places with fetch context. This presentation rounding never changes the raw provider token, exact ratio, persisted evidence or settlement status.

Salary / This Month may use either its existing dated manual `FundingPlanFX` or one applied `AlDarReferenceEvidence`, never both. The final owner correction makes explicit Refresh Al Dar independent of all Salary inputs and shortfall: one forward public `Home/GetRate` request always uses `currency=INR`, `amount=1.00` QAR and `isFCY=false`, producing a temporary `AlDarReferenceQuote` only. Use Reference constructs strict `AlDarReferenceEvidence` against the current positive INR shortfall; a missing or zero shortfall does not block lookup and cannot create applied durable evidence. Use Reference replaces manual authority in the draft; Save/Command-S persists. Manual rate editing clears external authority, including while the manual text is invalid. Dashboard observes only the saved canonical plan and never fetches. The latest owner/coordinator decision removes the Dashboard planning-FX card and its rate caption, superseding the earlier addition/rename. Manual fallback and Al Dar reference details stay inside the monthly planner. Dashboard retains only saved Salary/Funding estimates and never fetches or applies a reference. The future shared QAR/INR/USD market-FX authority remains distinct and unimplemented in Sprint 94.

The final owner clarification makes Salary account eligibility independent of institution and editable names: native QAR/INR, active runtime status, a nonempty durable repository ID and the typed role determine the set. Banks supply selectable planning balances; same-currency credit cards may be linked to manually entered commitments. NRE/NRO text is not subtype evidence or a planning restriction. The historical institution/account examples below do not impose a name or institution whitelist. The existing optional `fundingAccountID`/`funding_account_id` relationship already enforces same-workspace/native-currency membership and is not read by the calculator. New UI choices link cards; previously saved bank references remain unchanged and explicitly displayed as saved funding banks. No automatic card amount, recalculation rule, identity change, new field or migration is introduced.

`AlDarReturnedINRDecimal` retains the positive plain JSON numeric receive token separately from Money: at most 32 coefficient digits and 28 fractional digits. Zero, signs, exponents, quoted numbers, malformed responses and overflow reject. Persist the raw token, submitted native QAR Money, bound native INR shortfall, LedgerForge UTC fetch instant and fixed provider/source/direction/indicative classification. No provider timestamp, expiry, fee, quote ID or durable derived rate is invented. The ephemeral client uses normal HTTPS validation, no redirects, credentials/cookie/cache persistence, automatic retry, polling or startup fetch.

External calculation is `INR shortfall × submitted QAR ÷ raw returned INR`, exact through bounded full-width integer arithmetic, rounded upward only to the final QAR cent. Manual division/rounding and separate non-negative fee remain unchanged. Zero India shortfall produces zero principal and effective fee. Explicit application uses the fetched QAR 1 ratio for a planning estimate, retaining exact request/response precision; it does not claim a guaranteed rate for another transfer amount or trigger another request. Historical saved amount-specific evidence remains readable unchanged. The owner additionally selects `availableForInvestment = max(0, qarBeforeInvestment)`. `qarBeforeInvestment` and final QAR buffer remain signed; missing required inputs remain unavailable.

The final fixed-QAR-1 decision supersedes the former manual/applied/rollover seed order; no manual rate/date or new QAR input is required for lookup. A temporary unit quote may remain pending across financial-field edits and bind only on explicit Use. Manual FX rolls forward as before; external authority never does. A changed INR shortfall removes applied external authority and retains it only as temporary UI context. Saved evidence describes what the plan used, without an invented TTL or claim that it is current. Request identity, cancellation, plan identity and provider generation still reject obsolete replies. Untouched default-zero fields are visually blank with a 0 placeholder while meaningful zeros remain visible. Month labels use Aug 2026 format; the native date picker converts only the user-entered manual observation day, leaving imported source-date semantics unchanged.

Additive **V18** adds only `funding_plan_al_dar_references`, one PK/FK child per plan, plus reciprocal manual/external exclusion triggers. No `funding_plans` rebuild; V1–V17 stay byte-identical. Shared DTO validation checks lexical/Money identities, exclusivity and exact shortfall binding on save and external-reference staged hydration. Parent/child replacement is transactional. Request, generation and plan checks reject late replies across ownership changes; explicit application validates the current shortfall binding. [ADR-047](#adr-047) owns the approved V17 backup/receipt bridge.

This personal-use authorization covers current Salary planning only: no historical, global/net-worth/investment/reporting FX, reverse-rate inference, guaranteed settlement, fee inference, remittance execution or broader Salary expansion. [The work note](Work%20notes/Salary_and_current_AlDar.md) owns current evidence and limits.
...
## Historical future product direction — 2026-09-09
...
The current-holdings implementation is accepted under `SPRINT_96_INVESTMENT_IDENTITY_AND_CURRENT_HOLDINGS_ACCEPTED`, adds V21 and preserves format 1. Exact V17/V18/V19/V20 packages verify their original history/schema/manifest before an isolated candidate applies only its missing registered tail; V21 verifies directly. Source packages and original receipt identity remain unchanged. Populated investment backup/restore, complete hydration and a real isolated app relaunch passed; [the Sprint-96 work note](Work%20notes/Holdings_and_valuation.md#current-conclusion) owns the evidence and limits. Current remained V20 throughout qualification. Implementation commit `759be9fed485603bdea1bd9e8192372738233208` was published first, then the established bounded startup gate activated actual Current to V21 and verified clean relaunch with unchanged V1–V20 records and existing table contents; the [accepted outcome](Archive/Accepted%20outcomes/Sprints_90-99.md#sprint-96) records the exact sequence. No general historical migration bridge or new backup format is introduced.

## Current owner-approved alignment — Sprint 95, 2026-09-16

**Architecture and Sprint-95 implementation accepted** under `SPRINT_95_BUDGET_PLANNING_AND_SHARED_AL_DAR_ACCEPTED`; [accepted outcome](Archive/Accepted%20outcomes/Sprints_90-99.md#sprint-95). Format 1 retains its exact two-member package and existing content, activation, rollback and receipt ownership. Current accepted schema is V20. Exactly V17, V18 and V19 packages first pass their original manifest/hash, complete immutable history, schema, integrity and FK checks. Only an isolated candidate applies its missing V18/V19/V20 tail; current V20 verifies directly. Future/unknown/incomplete chains and V16-or-older remain unsupported. No general migration bridge is introduced.

Original package members remain unchanged and the receipt retains the original payload SHA-256, independently of the upgraded candidate identity. Canonical hydration, atomic plan evidence and relaunch confirmation remain required. The public two-leg FX cache and device appearance stay outside financial backup; effective evidence saved with a plan stays inside it. Existing receipt-owned V17/V18/V19 Current startup uses the same exact verified tail. A missing ledger remains unavailable and is never fabricated by recovery open. The [backup note](Work%20notes/Backup_and_export.md) and [Sprint-95 evidence](Work%20notes/Salary_and_current_AlDar.md#sprint-95-candidate) own actual results, including the owner-driven Xcode migration sequence.

## Historical accepted alignment — Sprint 94, 2026-09-15

The owner explicitly approved V18 compatibility while preserving format 1 and Sprint-93 container/content/activation behavior. New backups require exact V1–V18 history/schema; direct V18 restore is supported. Exactly V17 format-1 backups are supported through original manifest/size/hash, complete V1–V17 history and expected V17 schema verification; a separate operation-owned payload copy; only additive V18 migration; then V18 integrity, foreign-key, complete history/schema and canonical staged hydration checks before replacement confirmation. V16 and earlier, future versions, unknown/incomplete histories and incompatible formats remain rejected.

Neither the selected V17 package nor its operation-owned package copy is migrated. The separate prepared V18 candidate has an in-memory hash, rechecked before preservation and placement. The receipt retains the **original backup payload SHA-256**, not the upgraded candidate/live database hash. No financial rows or permanent ledger identity enter manifest/receipt metadata.

Startup resolves receipt authority before canonical open: pending preservation recovers the prior set; durable activation keeps the new set. A valid existing V17 canonical database with a retained Sprint-93 receipt may then pass exact V17 verification, apply only V18, hydrate, pass V18 BackupCompatibility verification and complete the receipt. Already-V18 startup verifies directly. Terminal receipts do not freeze valid existing V17 ledgers at V17. Missing, unknown or invalid state remains unavailable with assets retained; no recovery open creates an empty database. This bounded bridge does not authorize arbitrary historical backup migration.

The Sprint-94 implementation is owner/coordinator-accepted under `SPRINT_94_CURRENT_AL_DAR_AND_THIS_MONTH_PLANNING_ACCEPTED`; [the accepted outcome](Archive/Accepted%20outcomes/Sprints_90-99.md#sprint-94) owns the retained V18/V17 verification. Closure introduces no new ADR, migration or recovery execution. Sprint-93 accepted history and unobserved limits remain unchanged below.
