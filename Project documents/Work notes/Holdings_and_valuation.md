...
**Roadmap placement update — 2026-09-16.** Sprint 96 combines identity/revision foundations with current holdings through adopted manually qualified source paths. Each format still requires ordinary preparation, validation, account/instrument mapping, duplicate/equivalence handling, provider-owned persistence, canonical hydration and reopen before Sprint 98 uses it through email or parallel preparation. Sprint 97 separately qualifies prices/NAV and accepts the integrated current portfolio. Existing Direct-plan, exact identifier/plan/option, units, cost-form, provider permission, currency, source-date and manual-revision blockers remain unchanged.
