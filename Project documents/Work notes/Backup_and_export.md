...
The Sprint-93 outcome remains accepted. Sprint 94 is now owner/coordinator-accepted under `SPRINT_94_CURRENT_AL_DAR_AND_THIS_MONTH_PLANNING_ACCEPTED`, retaining the genuine isolated V17 upgrade/restore, V18 creation/direct restore, retained-receipt startup, selected rollback/interruption and actual Current Database migration/relaunch evidence. Closure publishes the unchanged accepted implementation without rerunning recovery or other verification. Format 1 supports exact V18 and an explicitly isolated V17→V18 bridge; V16/earlier/future/unknown history rejects. Original payload identity and the recorded recovery limits remain in force. Further schema changes still require an explicit compatibility decision. Complete structured export is OPTIONAL / NOT SELECTED; no restore-from-export workflow is implied. [Sprint-94 evidence](Salary_and_current_AlDar.md) owns exact current verification and non-runs.
