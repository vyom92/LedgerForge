...
<a id="sprint-95"></a>
## Accepted Sprint 95 — Budget Planning and Shared Al Dar FX — 2026-09-16

**OWNER/CHAT-ACCEPTED** under `SPRINT_95_BUDGET_PLANNING_AND_SHARED_AL_DAR_ACCEPTED`. Published predecessor and publication parent: `main@cdfb7565893c90a138ddbbfa8beebdfbad98f8f8`. The commit containing this record publishes the complete verified Sprint-95 product/test candidate and closure documentation together, including the previously merged nine-document Gmail/roadmap handoff. Closure adds no implementation, migration, ADR or credential change and reruns no completed build, test, source or recovery campaign. No app binary is run against Current during closure. Sprint 96 is **NEXT / NOT STARTED** and **PERSONAL-V1: NOT YET ADOPTED**.

### Accepted product outcome

- **Budget Planning** replaces the visible Salary destination. Sidebar order is Dashboard, Accounts, Budget Planning, Transactions, Import, Settings, with Budget Planning fitting one row. The monthly Qatar/India worksheet and retained Salary History use the saved appearance, readable text/alignment, shared themed actions and full section-tab hit areas. Separate Month and Year controls cover September 2026 through December 2099; session drafts survive month switching.
- The versioned worksheet uses explicitly pre-salary QAR cash, fixed + variable salary minus named deductions, Keep in CBQ, recurring Qatar/India commitments, temporary monthly exceptions preserving recurring basis, and optional bill dates recurring on the same day or the last day of a shorter month. Existing INR funds are explicitly optional. Available-to-send and India requirements remain distinct; exact one-fee semantics remain authoritative. There is no separate planned-investment subtraction or inferred bill payment/salary receipt. Live amount hints use lakh/crore. Save remains explicit.
- One shared Al Dar authority supplies direct QAR→INR and QAR→USD legs and exact local cross/inverse calculations. Two-decimal presentation preserves full internal precision. Opening/periodic updates, six-hour cadence, one retry after 60 seconds, independent retained successes, 24-hour/stale handling and truthful relative age remain as verified. Manual override stays month-local. Dashboard has no Refresh button; Budget Planning has themed Refresh with temporary feedback. Compact three-row flagged cards use one shared fetched-age indicator and the overall saved theme. The public cache remains separate from saved financial plan evidence.
- V19 worksheet state and additive V20 bill dates are accepted. **V20 is current; backup format 1 is unchanged.** Exact V17/V18/V19 packages retain original identity/receipt authority and upgrade only through the supported isolated missing migration tail; direct V20 restoration is supported. Prior migration identities remain unchanged. No general historical-migration bridge or new permanent ledger identifier is added.
- The Amex summary repair and separately implemented independent source oracle are accepted. Distinct Axis app/traditional saved credentials and the existing deduplicated bounded fallback are retained. The shared PDF reader and credential architecture are unchanged by the repair.

### Exact accepted verification

Accepted executable/source/test/build/resource identity: **337 inputs**, SHA-256 **`8bf71ad0fa97cabdd83f0143c644fb002fae51ba75f39aa87d4656905bfc4bf1`**. Closure documentation is outside this fingerprint; prepublication comparison confirms the accepted executable inputs are unchanged.

| Boundary | Accepted result and coverage |
| --- | --- |
| Ordinary TestPlan | **610 definitions / 674 executions; zero failures and zero runtime skips.** The existing **12 exclusion selectors / 17 separately gated definitions** remain unchanged. This ordinary run is not complete authentic-source certification. |
| Fresh builds | **Debug PASS; optimized Release (`-O`) PASS.** Deep/strict signatures, project plist and resource inventory pass. Resources contain only the app icon/assets, build identity and ZIPFoundation resources. |
| Amex focused and complete authentic family | **2/2 PASS:** the exact former `.malformedSummary` confirmation failure plus the complete **20-original** independent summary/transaction/section comparison. Existing multi/single-section confirmation, four source orders across both providers, cancellation/no-residue, supporting-source/exact-duplicate replay, canonical hydration and SQLite reopen pass. |
| Axis credit-card authentic family | **2/2 PASS:** complete **32-original** collection—18 app PDFs, seven traditional/email PDFs and seven XLSX—through independent comparison, ordinary confirmation, three source orders across both providers, replay, hydration and SQLite reopen. This campaign preceded only the last Amex-local prefix-bound guard; Axis product/test bytes were unchanged by that refinement. |
| Credential regressions | **25/25 PASS:** candidate order, equal-value deduplication, wrong-preferred/working-alternate success, exact-scope no-churn, independent rotations, compatibility preservation and failure/cancellation behavior. These overlap the ordinary plan and are not added to its unique-definition count. |
| Native preparation and confirmation | Repaired Amex original **47 transactions**, traditional Axis original **143 transactions**, each successfully prepared/validated/confirmed through the ordinary real credential path in an isolated memory ledger. The owner opened the selected originals after native file-picker automation could not enable Open; root observed preparation and completed confirmation. Both isolated sessions were closed. No native password-attempt telemetry is claimed. |
| Current and preferences after repair | Independent RAM comparisons retain exact equality across **56 Current tables**, schema and SQLite metadata, with zero missing/extra/changed records and unchanged saved appearance/profile preferences. No personal Budget Save, reset or discarded owner draft was used to pass repair checks. |
| Recovery | Genuine V17/V18/V19→V20 and direct-V20 recovery/relaunch checks passed as retained below and in the working history. No new recovery campaign is performed at publication. |

Final ordinary/build/source results are retained under `LedgerForge-S95-repair-dcr83114`: `focused/TestResults.xcresult`, `axis-password-gates.xcresult` and `final-cycle/test-full/TestResults.xcresult`. The [existing implementation/repair note](../../Work%20notes/Salary_and_current_AlDar.md#sprint-95-pdf-repair) owns exact artifact/fingerprint attribution and the earlier red/failed attempts. Native observations are recorded without copied statements, extracted-text files or disk financial oracles. Established isolated test databases were the only permitted financial test storage.

### Current migration sequence and recovery limits

Current was opened/migrated during Sprint-95 work **before the intended isolated qualification sequence had fully completed**. Specifically, the owner’s Xcode launch opened V19 before the subsequently requested recurring bill dates and later retained comparisons; V20 was therefore additive and V19 was not rewritten. Later genuine V17 and V18 packages upgraded in isolated product restore through V20 and clean relaunch, preserving respectively 51 and 52 existing tables and original identities. A genuine V19 package passed through V20 with 54 existing tables/columns preserved. Direct V20 recovery preserved all 56 tables/schema. Original package/payload identity and receipt ownership remained intact.

Later normal Current V20 startup and the retained comparisons do not constitute a before/after observation of the earlier owner-driven V18→V19 opening. That sequencing limitation remains accepted and explicit. The earlier inspection-sidecar mistake, its bounded cleanup and failed/setup runs remain in [working history](../../Work%20notes/Salary_and_current_AlDar.md#sprint-95-candidate). Publication neither mutates Current nor runs the old published V18 binary against V20. Previously unobserved power-loss/disk-full/per-file-interruption and related recovery cases remain unobserved; no repeated campaign or broader recovery certification is implied.

### Amex attribution and Axis credential boundary

The repaired Amex defect was a **brittle production assumption about PDFKit flattened-text ordering at the summary boundary**. The source table was coherent. The normalizer now establishes field ownership from the printed labels, table structure/geometry and currency relations before applying existing exact financial controls. The independent acceptance reader uses its own original `PDFPage` selections; it is separately implemented and does not reuse production output as financial truth. Missing/competing fields remain explicit failures. The earlier account/card confirmation defect is separate. macOS 27.0 (26A428) and Xcode 27.0 (27A266a) are the reproduced environment; no matched earlier-OS comparison establishes upgrade causality, and statement corruption is not asserted.

The saved scopes `axis-bank.credit-card.app-pdf` and `axis-bank.credit-card.traditional-pdf` remain distinct. The authorized repair corrected only the traditional scope from the already-proven existing compatibility value, preserving the app scope and compatibility credential. Fresh probes verified 18 app and seven traditional unlocks; values never enter documentation, logs or Git. The product tries the app candidate then the alternate when family metadata is unavailable, deduplicates equal values, and retries only for unlock failure. After successful unlock, parser/validation errors never trigger another password. Native alternate use is inferred from the same-source wrong-preferred/successful-alternate probe and deterministic ordinary preparation, rather than invented attempt telemetry. Closure makes no Keychain change.

### Retained warnings, history and future boundary

The final run retains 25 known LFTheme test-harness environment warnings and AppIntents metadata notices. Shell Xcode builds supply compiler verification; separate refreshed Xcode MCP per-file diagnostics were unavailable pending agent authorization. No recommended-settings batch was applied. Native file-picker and coordinate automation limitations are recorded with owner participation, not represented as automated coverage. Earlier 609/610-definition and 673/674-execution failures, the Amex diagnosis and intermediate repair failures remain history, superseded by the final green evidence above. The earlier full HDFC/CBQ-family results remain separately attributed; no broader rerun is claimed for the Amex-local repair. Historical UI/performance checkpoints are not recast as exact-final-fingerprint timing or a comprehensive accessibility campaign.

The already-merged **nine-document Gmail/roadmap handoff** is retained as future scope only. Sprint 95 implements no production Gmail intake. Combined HDFC/Axis email layouts remain **UNQUALIFIED for production import**; manual source qualification precedes automatic acceptance. One future explicit batch action may import only fully qualified/validated/resolved items while holding exceptions, with ordered independently atomic commits. Automatic startup/polling/background fetching remains **DEFERRED**. Gmail pilot limits, source restrictions and the separate parser-alternatives idea remain unchanged.

The settled sequence remains **96 investment identity/current holdings through manual source qualification → 97 current valuation/integrated portfolio acceptance → 98 manual Gmail/combined-bank qualification and bounded batch intake → 99 reporting conversion/net worth → 100 personal-v1 adoption verification**. Sprint 96 is next and not started. Accepted Sprint 95 completes the selected `FW-P3-08` planning workflow, selected `FW-P3-31` shared Al Dar authority and `FW-P1-40` Amex defect. Only bounded planning/obligation/presentation slices under `FW-P3-09/30` are accepted; broader forecasting, reporting/net worth, salary reconciliation and unrelated future work retain their separate status. Completed IDs remain in [SCOPE_DECISIONS](../../SCOPE_DECISIONS.md#consolidation-and-completed-id-references).

The exact **55-path published product/test/documentation inventory** is retained in the [existing owning note](../../Work%20notes/Salary_and_current_AlDar.md#accepted-publication-file-inventory). The final commit is the commit introducing this acceptance record, avoiding a self-referential hash placeholder. No private original, OAuth client/token, statement bytes, database, coverage artifact, task scratch or credential is included. Closure ends after exact normal publication and fresh local/remote/clean-state verification; it does not start Sprint 96 or personal adoption.
