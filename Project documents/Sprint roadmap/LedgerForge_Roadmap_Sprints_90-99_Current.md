...
| 96 | Investment Identity and Current Holdings | completed selected `FW-P3-20/22/23`; holdings slices of `FW-P3-21/29` | **ACCEPTED**, 2026-09-17; V21, qualified current-position manual imports, 41 holdings / 15 containers and owner-verified table; [accepted outcome](../Archive/Accepted%20outcomes/Sprints_90-99.md#sprint-96) |
| 97 | Current Valuation and Integrated Current-Portfolio Acceptance | bounded accepted 96 outputs plus `FW-P3-21/27`, selected `FW-P3-28`, `FW-P3-29` | **NEXT / NOT STARTED.** Independently qualified prices/NAV, then former Sprint-98 integrated acceptance: exact units, typed cost, provenance, provider parity, hydration/reopen and truthful incomplete state |
...
**ACCEPTED, 2026-09-17**, under `SPRINT_96_INVESTMENT_IDENTITY_AND_CURRENT_HOLDINGS_ACCEPTED`, from published predecessor `main@2f299b1ced7586fafb390e760dcac56e378e75d3`. The [accepted outcome](../Archive/Accepted%20outcomes/Sprints_90-99.md#sprint-96) owns 36-original comparison, 613/679/0/0 ordinary regression, bank/card and salary regressions, Debug/Xcode/Release, populated V21 recovery and owner column-drag verification. Two source cases remain UNOBSERVED with structural review: absence-only exit and newer missing cost clearing a known cost. Publication precedes bounded Current activation; no new implementation is selected by closure.

Accepted owners: selected [FW-P3-20](../FUTURE_WORK.MD#fw-p3-20), [FW-P3-22](../FUTURE_WORK.MD#fw-p3-22), [FW-P3-23](../FUTURE_WORK.MD#fw-p3-23) and the current-holdings portions of [FW-P3-21](../FUTURE_WORK.MD#fw-p3-21) and [FW-P3-29](../FUTURE_WORK.MD#fw-p3-29). Their current valuation remainder stays in Sprint 97.

The owner approved the [simplified current-holdings contract](../Work%20notes/Holdings_and_valuation.md#current-conclusion): small container/current-holding state, exact units and separately reported costs, confirmed aliases, scoped atomic updates, existing import provenance and canonical provider/hydration ownership. The earlier historical observation/revision graph is superseded. Current holdings use the qualified genuine manual source paths. Each adopted format must pass ordinary preparation, validation, account/instrument mapping, duplicate/equivalence handling, provider-owned persistence, canonical hydration and reopen before Sprint 98 uses it through email or parallel preparation. Genuine owner/source evidence and Chat architecture acceptance remain entry gates.

Require exact owned units, holdings dates, separate source NAV/price dates, existing import provenance and separately reported average/total acquisition cost. Preserve source precision with the bounded investment decimal; absent cost stays unavailable. Direct-plan mutual-fund plan/option identity and separate mandatory/AVC/employer ISP policy scopes remain exact requirements. The new ISP closing reports remove the need for a manual holdings fallback. No contribution allocation, historical observation graph, trade ledger or split reconstruction is selected. Prices/NAV never establish ownership.

The [owning holdings note](../Work%20notes/Holdings_and_valuation.md#packet-investment-holdings-architecture-packet) retains accepted source rules, exact identity/cost meaning and separate future provider findings. The accepted current-holdings contract uses additive V21 with V1–V20 unchanged; the [accepted outcome](../Archive/Accepted%20outcomes/Sprints_90-99.md#sprint-96) records publication and actual Current activation order.

### Sprint 97 — Current Valuation and Integrated Current-Portfolio Acceptance

**NEXT / NOT STARTED.** No Sprint-97 implementation is authorized by Sprint-96 closure.
...
<a id="sprint-98-manual-gmail-and-combined-bank-intake"></a>
...
<a id="sprint-98-qualification-and-adoption-population"></a>
**Required acceptance and adoption-preparation campaign — owner direction, 2026-09-16.** After the production manual Gmail and bounded-parallel batch capabilities are implemented, execute the [independent source qualification contract](../Work%20notes/Source_relationships.md#sprint-98-source-qualification) and [lifecycle/final-population campaign](../Work%20notes/Account_and_document_lifecycle.md#sprint-98-adoption-population) as part of Sprint 98. This is intended future acceptance work, not another parser benchmark, a new sprint or current implementation authorization.

- Exercise every selected authentic original in each supported family through the normal production path and compare independently established source meaning field by field **after persistence/hydration**. Use a high-capability Codex examiner with source interpretation established before inspecting parsed output where practicable; disagreements trigger investigation, not model authority.
- Compare bank/card occurrences, ownership, exact amounts/date roles, references/narrations and controls under the whole-and-unique matching rule; source order/provenance remains preserved. Investment comparison concerns current holdings, exact units, source-reported typed costs, as-of dates, scoped revisions and replay, plus Sprint-97-qualified current valuation; it does not create transaction-history reconstruction or shift the existing Sprint-96/97 gates.
- Prove serial/bounded-parallel equivalence from logically identical isolated starting states, ordered commit-time mapping/duplicate rechecks, bounded memory/worker lifetime, throughput and normal native UI responsiveness. Preserve the existing single commit lane and manual batch authorization below.
- Investigate discrepancies by source evidence, apply only source-certain narrow corrections, refer unresolved financial meaning to the owner and rerun the complete affected authentic family plus relevant ordinary regression. A2/B2 remain source-specific; no generic one-paise tolerance or generic multi-engine selector is introduced.
- Freeze the qualified implementation, use the selected clean adoption-candidate state, then populate the complete approved historical/current source set through that one frozen implementation. Explicitly hold unsupported, internally inconsistent or unresolved originals and reconcile the final persisted data and coverage. No mixed-parser-version investigation database becomes the adoption candidate.
- Record requested versus available coverage, missing periods, excluded/nonfinancial inputs, supported/unsupported layouts, independently verified imports and held exceptions. The owner may authorize institution/family-specific older lookback at execution; this note fixes no January-2025 lower bound and processing all available email alone is not completeness.

The resulting populated adoption candidate passes to Sprint 99 reporting/net worth and Sprint 100 whole-app verification. This note grants no present database reset, Gmail discovery, parser change or adoption declaration.
...
Consume the populated adoption candidate produced by the [Sprint-98 frozen qualification/population campaign](#sprint-98-qualification-and-adoption-population), retaining its coverage and held-source limitations. This handoff does not itself declare adoption.
