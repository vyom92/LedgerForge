...
**Accepted Sprint-92 presentation extension — 2026-09-14:** [SC-05A](LF-UI-2026-09-R1_SC-05A_Cross_Screen_Conformance_Matrix.md#sprint-92-refresh) records the refreshed residuals. Accounts reuses shared financial colours with existing amount/sign and typed liability-effect meaning. Import completion review uses the actual terminal outcome/recovery state instead of an obsolete preparation-empty prompt; the owner removed the repeated local-processing banner. Dashboard durable import, Import History and prior-import event timestamps use the owner's selected Mac-local date/time with an explicit UTC offset; missing/unparseable instants show “Time unavailable”. This does not convert financial civil dates or substitute import time for a balance/transaction date. Salary uses “Salary actuals”, “Planning rate (INR per QAR)”, display-only tabular Money and Remove commitment help while preserving every input, fee, draft, calculation and Save rule. Import Preview placement remains unchanged because no genuine prepared state was available to select A/B. Settings and Developer Console retain the accepted behavior. Native branch/width limits and final build identity live in [the accepted outcome](../../Archive/Accepted%20outcomes/Sprints_90-99.md#sprint-92). Broader FW-P2-46 charts and the FW-P2-48 Preview A/B proposal remain open; unavailable native states are not retroactively certified. Regression tests remained suspended with zero new executions.
