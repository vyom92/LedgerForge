...
Later accepted boundaries include exact source-byte authority, source-specific equivalence/lineage, categories, card evidence, Salary and the authentic-source reset; see their numeric [ADRs](ADR.md) and accepted outcomes via current state. The owner’s [current processing decision](SCOPE_DECISIONS.md#source-processing-decision) preserves the normal app database but prohibits derived financial evidence files; old on-disk fixture/oracle recipes are superseded. Product backup/restore is accepted under [ADR-047](ADR.md#adr-047), with exact current compatibility owned by PROJECT_STATE. Encryption, sync and multiple workspaces are not prerequisites.

### Accepted Sprint-96 current-holdings boundary — 2026-09-17

The current-holdings-only implementation is **ACCEPTED** under `SPRINT_96_INVESTMENT_IDENTITY_AND_CURRENT_HOLDINGS_ACCEPTED` with additive V21. A small container retains actual portfolio identity, confirmed aliases and accepted scope date; its current holdings retain exact units, separate optional unit/total cost and existing document/session/fingerprint provenance. Provider-owned confirmation revalidates and atomically replaces only established coverage, then canonical hydration publishes the complete state. Historical zero positions and separate investment history/revision/event tables are excluded. Current-position replacement does not change the immutable bank/card transaction rules below. [Holdings and valuation](Work%20notes/Holdings_and_valuation.md#current-conclusion) owns the accepted contract, source rules and qualification. Current remained V20 during qualification. After implementation publication, the established startup workflow activated actual Current to V21 and verified full hydration/clean relaunch while preserving existing records; the accepted outcome records that sequence. Closure imports no qualification data into Current.
