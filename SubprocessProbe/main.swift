...
struct LedgerForgeSubprocessProbe {
    static func run() {
        ...
        let normalizedRow = NormalizedRowDTO(id: "probe-normalized-row-\(scenario)-\(suffix)", normalizedDocumentId: normalizedDocument.id, sourceOrdinal: 1, digest: String.normalizedRecordDigest(values: [scenario, suffix]))
        let transaction = TransactionDTO(id: transactionID, workspaceId: workspace.id, postedDateISO: "2026-07-20", nativeCurrency: "INR", amountMinor: 100, amountDecimal: "1.00", direction: "debit", createdAtISO: now, rawRows: [TransactionRawRowDTO(id: "probe-raw-row-\(scenario)-\(suffix)", normalizedRowId: normalizedRow.id, contributionType: "transaction", sourceOrdinal: 1, normalizedRecordDigest: normalizedRow.digest, normalizedDocumentId: normalizedDocument.id)])
